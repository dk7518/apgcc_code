...
class AttentionDecoder(nn.Module):
    def __init__(self, in_planes, num_anchor_points=4, num_classes=2, inner_planes=256, feat_layers=[3, 4], **kwargs):
        ...
        # Attention Mechanism
        self.attention = nn.MultiheadAttention(embed_dim=self.inner_planes, num_heads=4)

        # Classification and Regression Heads
        from .modules import RegressionModel, ClassificationModel
        self.offset_head = RegressionModel(num_features_in=self.inner_planes, num_anchor_points=num_anchor_points)
        self.confidence_head = ClassificationModel(num_features_in=self.inner_planes, num_classes=num_classes, num_anchor_points=num_anchor_points)

    def forward(self, samples, features):
        # Feature extraction using FPN
        features_fpn = self.fpn([features[0], features[1], features[2], features[3]])
        
        # Use last layer features for attention
        batch_size, _, height, width = features_fpn[-1].shape
        fpn_output = features_fpn[-1].view(batch_size, self.inner_planes, -1).permute(2, 0, 1)  # [Seq_len, Batch, Channels]

        # Self-Attention
        attention_output, _ = self.attention(fpn_output, fpn_output, fpn_output)
        attention_output = attention_output.permute(1, 2, 0).view(batch_size, self.inner_planes, height, width)

        # Heads
        offset = self.offset_head(attention_output) * 100
        confidence = self.confidence_head(attention_output)

        return {'pred_logits': confidence, 'pred_points': offset}
